...
from app.infrastructure.persistence.document_repository import PostgresDocumentRepository
...
from app.infrastructure.persistence.section_repository import PostgresSectionRepository
...
class DIContainer:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._section_repo: AbstractSectionRepository | None = None
    ...
    def chunk_repo(self) -> AbstractChunkRepository:
        if self._chunk_repo is None:
            self._chunk_repo = PostgresChunkRepository(self._session_factory)
        return self._chunk_repo

    @property
    def section_repo(self) -> AbstractSectionRepository | None:
        if self._section_repo is None:
            try:
                self._section_repo = PostgresSectionRepository(self._session_factory)
            except Exception:
                return None
    ...
    def task_queue(self) -> AbstractTaskQueue:
        if self._task_queue is None:
            if self._settings.storage.redis_url:
                ...
                redis_client = AsyncRedis.from_url(self._settings.storage.redis_url)
    ...
    def _pipeline_steps(self) -> list:
        ...
        steps = [
            ...
            RetrievalStep(self.embed_service, self.vector_repo, self.search_service, self.cache_service, self._settings),
    ...
    def query_use_case(self) -> QueryUseCase:
        if self._query_use_case is None:
            ...
            self._query_use_case = QueryUseCase(orchestrator, self.cache_service, self.doc_repo, self._settings)
    ...
    def stream_use_case(self) -> StreamingQueryUseCase:
        if self._stream_use_case is None:
            ...
            self._stream_use_case = StreamingQueryUseCase(
                orchestrator=orchestrator, generation_step=self._generation_step,
                cache=self.cache_service, document_repo=self.doc_repo, settings=self._settings,
            )
