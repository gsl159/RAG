...
class OpenAIProvider:
    """OpenAI-compatible API provider for chat completions and embeddings."""
        ...
        **kwargs,
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"OpenAI HTTP {resp.status_code} on attempt {attempt + 1}/{max_retries}"
                )
            except (httpx.TimeoutException, httpx.ConnectError) as e:
                last_exc = e
                logger.warning(
                    f"OpenAI {type(e).__name__} on attempt {attempt + 1}/{max_retries}"
                )
    ...
    async def embed_batch(
        ...
        for i in range(0, len(texts), batch_size):
            ...
            logger.debug(
                f"OpenAI embedding batch {i}~{i + len(batch)}/{len(texts)}"
            )
            resp = await self._request_with_retry(
                "POST",
